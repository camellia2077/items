...
- rules in `RandomLoadout.rules.json` support:
  - `random`
  - `specific`
- `specific` rules support either:
  - `name`: exact primary display name, case-insensitive
  - `alias`: shared alias from `RandomLoadout.aliases.json`
...
- `RandomLoadout.aliases.json` is the shared readable alias layer that points back to `pickupId`
